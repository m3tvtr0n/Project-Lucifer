...
scan_networks() {
    ...
    if [ "$T_ESSID" = "<hidden>" ] || [ -z "$T_ESSID" ]; then
        ...
        # Method 1: passive — check if any client probe reveals it
        resolved_ssid=$(timeout 10 tcpdump -i "$MON_TARGET" -e -c 20 \
            "ether dst $T_BSSID and type mgt subtype probe-resp" 2>/dev/null \
            | grep -oP 'SSID=\K[^,]+' | head -1)

        # Method 2: aireplay directed probe (triggers probe response)
        if [ -z "$resolved_ssid" ]; then
            aireplay-ng -9 -a "$T_BSSID" -e "" "$MON_TARGET" \
                >/dev/null 2>&1 &
            local probe_pid=$!

            resolved_ssid=$(timeout 10 tcpdump -i "$MON_TARGET" -e -c 5 \
                "ether src $T_BSSID and type mgt subtype probe-resp" \
                2>/dev/null \
                | grep -oP 'SSID=\K[^,]+' | head -1)
